...
//
FileProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines, assetSearchPath,base) {
  ...
  regexps.forEach(function (rxl) {
    ...
    content = content.replace(rxl[0], function (match, src) {
      ...
      var destFile=file;
      if(base!=undefined) {
          if(base.substr(0,2)=='//'||base.substr(0,7)=='http://'||base.substr(0,8)=='https://'){
              destFile=url.resolve(base,file);
          }else{
              destFile=path.join(base,file);
          }
      }
      debug('Found file \'%s\'', destFile);

      if (src === '/') {
        return match;
      }

      var res = match.replace(src, filterOut(destFile));
      if (srcFile !== file) {
        self.log(chalk.cyan(src) + ' changed to ' + chalk.cyan(destFile));
      }
      return res;
    });
  });
...
FileProcessor.prototype.process = function (filename, assetSearchPath,base) {
  ...
  var content = this.replaceWithRevved(this.replaceBlocks(this.file), this.file.searchPath,base);
